...
export function ConnectOneDrive({
  ...
  async function handleConnect() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/onedrive/authorize");
      if (!res.ok) throw new Error("Failed to get authorize URL");
      const { url } = await res.json();
      window.location.href = url;
    } catch {
      setError("Failed to connect OneDrive");
      setLoading(false);
    }
  }
  ...
  async function handleDisconnect() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/onedrive/connect", { method: "DELETE" });
  ...
  return (
    <div className="flex flex-col gap-3">
      <button
        ...
        disabled={loading}
        className="flex items-center justify-center gap-2 rounded-lg bg-[#0078d4] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#106ebe] disabled:opacity-50"
      >
